...
class TestClassifySignatureChange:
    ...
    def test_parameter_added_breaking(self):
        assert classify_signature_change("def f(a)", "def f(a, b)") == "PARAMETER ADDED (REQUIRED)"

    def test_parameter_added_nonbreaking(self):
        assert classify_signature_change("def f(a)", "def f(a, b=1)") == "OPTIONAL PARAMETER ADDED"
    ...
    def test_kwonly_added_breaking(self):
        assert (
            classify_signature_change("def f(a)", "def f(a, *, k)") == "PARAMETER ADDED (REQUIRED)"
        )
...
def _init_repo(tmp_path):
    ...
    subprocess.run(
        ["git", "config", "core.hooksPath", str(tmp_path / ".git" / "hooks")],
        cwd=repo,
        capture_output=True,
        check=True,
    )
...
class TestRicherCategoriesCLI:
    ...
    def test_parameter_added_breaking_category(self, tmp_path):
        ...
        assert data["findings"][0]["category_id"] == "required_parameter_added"
    ...
    def test_return_type_changed_category(self, tmp_path):
        ...
        result = runner.invoke(main, ["review", "HEAD~1..HEAD", "--repo", repo])
        assert result.exit_code == EXIT_FINDINGS
        assert "RETURN ANNOTATION CHANGED" in result.output
    ...
    def test_json_review_hints_specific(self, tmp_path):
        ...
        result = runner.invoke(main, ["review", "HEAD~1..HEAD", "--repo", repo, "--format", "json"])
        data = json.loads(result.output)
        assert "required parameter" in data["findings"][0]["review_hint"]
...
class TestInstallHook:
    ...
    def test_install_pre_commit(self, tmp_path):
        ...
        assert hook_path.exists()
        hook_text = hook_path.read_text()
        assert "diffguard review --staged" in hook_text
        assert "diffguard review HEAD" not in hook_text
        assert "elif [ $status -ne 0 ]" in hook_text
        assert "blocking commit" in hook_text

    def test_install_from_nested_directory_uses_repository_hooks(self, tmp_path):
        _init_repo(tmp_path)
        ...
        result = CliRunner().invoke(
            main,
            ["install-hook", "--repo", str(nested), "--hook-type", "pre-commit"],
        )
